...
namespace Ecommerce.Api.Controllers.Identity
{
    ...
    public class AddressController : ControllerBase
    {
        ...
        public async Task<IActionResult> AddAddress([FromBody] CreateAddressRequestDto dto)
        {
            ...
            var address = await _addressService.CreateAddressAsync(dto, GetUserId());
        ...
        public async Task<IActionResult> UpdateAddress(Guid addressId, [FromBody] CreateAddressRequestDto dto)
        {
            if (dto == null) return BadRequest(new { message = "Address data is required." });
            try
            {
                var address = await _addressService.UpdateAddressAsync(GetUserId(), addressId, dto);
                return Ok(new { message = "Address updated successfully", data = address });
            }
